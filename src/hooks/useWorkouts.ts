...
const useWorkouts = () => {
  ...
  const workoutData: WorkoutItem[] = [
    ...
    { bodyPart: "Legs", exercises: ["Squats", "Lunges", "Leg Curls", "Leg Press"] },
      ...
      const workoutPayload: BackendWorkoutPayload = {
        ...
        workouts: {
          [values.selectedBodyPart]: {
            exercises: values.workouts.reduce((acc: BackendExercise[], workout) => {
              const existingExercise = acc.find(ex => ex.name === workout.exercise);
              const set = {
                kg: workout.kg,
                reps: workout.reps,
                time: workout.time
  ...
  const removeWorkout = (index: number) => {
    formik.setFieldValue("workouts", formik.values.workouts.filter((_, i) => i !== index));
  };
